...
int main()
{
    ...
    float x1 = -10, y1 = 5, // Координаты первой точки прямой
        x2 = 26, y2 = 4,    // Координаты второй точки прямой
        x3 = 30, y3 = -16,  // Координаты третьей точки
        x4 = -35, y4 = 40;  // Координаты четвёртой точки
    ...
    while (TRUE) { // Бесконечный цикл для работы визуальной отрисовки
        ...
        if (visualMode) { // Если включен режим визульной отрисовки, то выполняются следующие функции
            ...
            if (y4 < k2 * x4 + b2 && y4 > k3 * x4 + b3 && y4 > k1 * x4 + b1) sectorNumber = 1;
            if (y4 < k2 * x4 + b2 && y4 < k3 * x4 + b3 && y4 < k1 * x4 + b1) sectorNumber = 2;
            if (y4 > k2 * x4 + b2 && y4 < k3 * x4 + b3 && y4 < k1 * x4 + b1) sectorNumber = 3;
            if (y4 > k2 * x4 + b2 && y4 > k3 * x4 + b3 && y4 > k1 * x4 + b1) sectorNumber = 4;
            if (y4 < k2 * x4 + b2 && y4 > k3 * x4 + b3 && y4 < k1 * x4 + b1) sectorNumber = 5;
            if (y4 < k2 * x4 + b2 && y4 < k3 * x4 + b3 && y4 > k1 * x4 + b1) sectorNumber = 6;
            if (y4 > k2 * x4 + b2 && y4 > k3 * x4 + b3 && y4 < k1 * x4 + b1) sectorNumber = 7;

            // Если треугольник не построился
            if (sectorNumber == 0) printf("Error: triangle does not exist\n");
            ...
            for (int i = -100; i < 100; i++)
            {
                ...
                {
                    if (sectorNumber == 1) if (j < k2 * i + b2 && j > k3 * i + b3 && j > k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 1
                    if (sectorNumber == 2) if (j < k2 * i + b2 && j < k3 * i + b3 && j < k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 2
                    if (sectorNumber == 3) if (j > k2 * i + b2 && j < k3 * i + b3 && j < k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 3
                    if (sectorNumber == 4) if (j > k2 * i + b2 && j > k3 * i + b3 && j > k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 4
                    if (sectorNumber == 5) if (j < k2 * i + b2 && j > k3 * i + b3 && j < k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 5
                    if (sectorNumber == 6) if (j < k2 * i + b2 && j < k3 * i + b3 && j > k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 6
                    if (sectorNumber == 7) if (j > k2 * i + b2 && j > k3 * i + b3 && j < k1 * i + b1) TextOut(hDC, 500 + 5 * i, 400 - 5 * j, "-", sizeof("-")); // 7
                }
            }
